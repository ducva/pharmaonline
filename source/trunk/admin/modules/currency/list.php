...
<?php	$list=new XTemplate('html/modules/currency/list.tpl');
	
	$sLink="?page=currency&do=list";
	...
	switch($sDo)
	{
		...
		case 'status':
				...
				{
					$iStatus= 1 - $iStatus;
					$currency->update('Status='.$iStatus,'Id='.$_GET['id']);
	...
	$aCurrency=$currency->select('','','',$sCondition,'Position asc');
		...
		for($i=0;$i<$iLine;$i++)
		{
			...
			{	
				...
				$aArray[$i]= array(
									...
									'INFO'=>$aCurrency[$i]['Info'],
									'RATE'=>$aCurrency[$i]['Rate'],
									'STATUS'=>$aCurrency[$i]['Status'],
									'LOCK'=>$sLock );
				$list->assign('LISTCURRENCY',$aArray[$i]);
				$list->parse('MAIN.LISTCURRENCY');
			}
		}
