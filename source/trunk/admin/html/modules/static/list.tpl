...
<TABLE class=center cellSpacing=0 cellPadding=0>
	<TBODY>
  		<TR>
    		<TD vAlign=top>
      		<FORM action="?page=static&do=list" name="frmNews" method="post">
...
<INPUT class=button type=reset value="Nhập lại"> 
									...
									<TR  onmouseover=change_color(this,7) onmouseout=delete_color(this,7)>
                      					<TD align=middle>{LISTCONTENTS.ID}</TD>
                     					<TD align=left style="padding-left:5px;">{LISTCONTENTS.TITLE} 										</TD>
                      					<td align="middle"><img src="{LISTCONTENTS.IMAGE}" /></td>
                      					<td align="middle">{LISTCONTENTS.NEWSCATEGORY}</td>
										<td align="middle">{LISTCONTENTS.WRITEDATE}</td>
                      					<TD align=middle>{LISTCONTENTS.USERNAME}</TD>
                      					<TD align=middle>
											<A title=Khóa href="?page=news&do=list&action=status&id={LISTCONTENTS.ID}&status={LISTCONTENTS.STATUS}">
												<IMG class=icon src="{LISTCONTENTS.LOCK}"></A>&nbsp;
											<A title=Sửa href="?page=news&do=edit&id={LISTCONTENTS.ID}">
